...
#if !defined(__MK20DX256__) && !defined(__IMXRT1062__)
#error "Wrong target platform. Please set to Teensy 3.1/3.2 (MK20DX256) or Teensy 4.0 (IMXRT1062)."
#endif
...
unsigned short extraSrc;           // LIP:GLS
...
void setup() {
    ...
    if (widiJumper && widiOn)
        digitalWrite(widiPowerPin, HIGH);
    else
        digitalWrite(widiPowerPin, LOW);
    ...
    if (mainState == NOTE_OFF) {
        ...
        if (legacy || legacyBrAct) {
            ...
            if (
                ...
                (brSuck && legacyBrAct && bcasMode && noPb)) { // both pb pads touched or br suck

                int fingeredNoteUntransposedForPatch = patchLimit(fingeredNoteUntransposed + 1);
                if (exSensorUse >= ((extracThrVal + extracMaxVal) / 2)) { // instant midi setting
        ...
        if (analogRead(breathSensorPin) > (breathCalZero - 850))
            programonce = false;
...
static void sendHarmonizerData(uint8_t note, const int harmony[][3], bool sendOn) {
    ...
    if (sendOn) {
        ...
        if (hmzLimit > 2)
            midiSendNoteOn(noteValueCheck(note + offs[1]), velocitySend);
        if (hmzLimit > 3)
            midiSendNoteOn(noteValueCheck(note + offs[2]), velocitySend);
    } else {
        midiSendNoteOff(noteValueCheck(note + offs[0]));
        if (hmzLimit > 2)
            midiSendNoteOff(noteValueCheck(note + offs[1]));
        if (hmzLimit > 3)
            midiSendNoteOff(noteValueCheck(note + offs[2]));
    }
}
...
static void updateRotator(byte note, const Rotator *rotator) {
    ...
    if (rotations[currentRotation] - 24)
        midiSendNoteOn(noteValueCheck(note + rotations[currentRotation] - 24), velocitySend); // send Note On message for new note
}

//**************************************************************

static void stopRotatorNotes(byte note, const Rotator *rotator) {
    if (rotator->parallel - 24)
        midiSendNoteOff(noteValueCheck(note + rotator->parallel - 24)); // send Note Off message for old note
    if (rotator->rotations[currentRotation] - 24)
        midiSendNoteOff(noteValueCheck(note + rotator->rotations[currentRotation] - 24)); // send Note Off message for old note
}

static void stopHarmonizerNotes(byte note) {
    switch (polySelect) {
    case ETriadMajorGospelRoot:
        sendHarmonizerData(note, majGosRootHmz, false);
        break;
    case ETriadMajorGospelDominant:
        sendHarmonizerData(note, majGosDomHmz, false);
        break;
    case EMajorAddNine:
        sendHarmonizerData(note, majAdd9Hmz, false);
        break;
    case EMinorDorian:
        sendHarmonizerData(note, minDorHmz, false);
        break;
    case EMinorAeolian:
        sendHarmonizerData(note, minAeoHmz, false);
        break;
    case EMinorFourVoiceHip:
        sendHarmonizerData(note, minHipHmz, false);
        break;
    case EFourWayCloseHarmonizer: {
        if (!fwcDrop2 || (hmzLimit > (3 + fwcLockH)))
            midiSendNoteOff(noteValueCheck(note + blockFWC[fwcType][(note - hmzKey) % 12][0] - 12 * fwcDrop2));
        if ((hmzLimit + fwcDrop2) > 2)
            midiSendNoteOff(noteValueCheck(note + blockFWC[fwcType][(note - hmzKey) % 12][1]));
        if ((hmzLimit + fwcDrop2) > 3)
            midiSendNoteOff(noteValueCheck(note + blockFWC[fwcType][(note - hmzKey) % 12][2]));
        if (((hmzLimit + fwcDrop2) > 4) && (1 == fwcLockH))
            midiSendNoteOff(noteValueCheck(note - 12));
    } break;

    case ERotatorA:
        stopRotatorNotes(note, &rotations_a);
        break;
    case ERotatorB:
        stopRotatorNotes(note, &rotations_b);
        break;
    case ERotatorC:
        stopRotatorNotes(note, &rotations_c);
        break;
...
static void startHarmonizerNotes(byte note) {
    switch (polySelect) {
    case ETriadMajorGospelRoot:
        sendHarmonizerData(note, majGosRootHmz, true);
        break;
    case ETriadMajorGospelDominant:
        sendHarmonizerData(note, majGosDomHmz, true);
        break;
    case EMajorAddNine:
        sendHarmonizerData(note, majAdd9Hmz, true);
        break;
    case EMinorDorian:
        sendHarmonizerData(note, minDorHmz, true);
        break;
    case EMinorAeolian:
        sendHarmonizerData(note, minAeoHmz, true);
        break;
    case EMinorFourVoiceHip:
        sendHarmonizerData(note, minHipHmz, true);
        break;
    case EFourWayCloseHarmonizer: {
        int limit = (hmzLimit + fwcDrop2);
        if (!fwcDrop2 || (hmzLimit > (3 + fwcLockH)))
            midiSendNoteOn(noteValueCheck(note + blockFWC[fwcType][(note - hmzKey) % 12][0] - 12 * fwcDrop2), velocitySend);
        if (limit > 2)
            midiSendNoteOn(noteValueCheck(note + blockFWC[fwcType][(note - hmzKey) % 12][1]), velocitySend);
        if (limit > 3)
            midiSendNoteOn(noteValueCheck(note + blockFWC[fwcType][(note - hmzKey) % 12][2]), velocitySend);
        if ((limit > 4) && (1 == fwcLockH))
            midiSendNoteOn(noteValueCheck(note - 12), velocitySend);
    } break;

    case ERotatorA:
        updateRotator(note, &rotations_a);
        break;
    case ERotatorB:
        updateRotator(note, &rotations_b);
        break;
    case ERotatorC:
        updateRotator(note, &rotations_c);
        break;
...
void pitch_bend() {
    ...
    if (1 == leverControl) { //lever vibrato
        ...
        if (allPiezo) { //Using the piezo voltage and zeroing directly
            if (UPWD != vibDirection)
                piezoVib = -(vibRead - vibZero) * vibSens * 10 * vibDepth[vibrato];
            else
                piezoVib = (vibRead - vibZero) * vibSens * 10 * vibDepth[vibrato];
            if (abs(piezoVib) < (vibSquelch * vibSens * 10 * vibDepth[vibrato]))
                piezoVib = 0;
        } else { //Use delta like with capacitive and bite sensor
            if (vibRead < vibThr) {
                if (UPWD == vibDirection) {
...
// Extra Controller is the lip touch sensor (proportional) in front of the mouthpiece
    ...
    if ((harmSetting && (pinkySetting != ECH)) || ((pinkySetting == ECH) && pinkyKey)) {
        if (harmSelect < 4) {
            harmonics = map(constrain(exSensorUse, extracThrVal, extracMaxVal), extracThrVal, extracMaxVal, 0, harmSetting);
        } else {
            harmonics = map(constrain(exSensorUse, extracThrVal, extracMaxVal), extracMaxVal, extracThrVal, 0, harmSetting);
        }
    } else if ((pinkySetting == ECH) && !pinkyKey) {
        harmonics = 0;
    }

    if ((extraCT || extraCT2) && (exSensorUse >= extracThrVal)) { // if we are enabled and over the threshold, send data
        if (!extracIsOn) {
            extracIsOn = 1;
        ...
        if ((extraCT == 1) && CC1sw) { //Send modulation
            extracCC = map(constrain(exSensorUse, extracThrVal, extracMaxVal), extracThrVal, extracMaxVal, 1, 127);
...
void autoCal() {
    ...
    if (calReadNext > calRead)
        calRead = calReadNext; //use highest value
#else
...
void readSwitches() {
    ...
    case 0:
        ...
        if (R5 && R1)
            octaveR = 6; //R6 = R5 && R1
        else if (R5 && (R4 || oneroll))
            octaveR = 5; //R5
        else if (R4 && (R3 || oneroll))
            octaveR = 4; //R4
        else if (R3 && (R2 || oneroll))
            octaveR = 3; //R3
        else if (R2 && (R1 || oneroll))
            octaveR = 2; //R2
        else if (R1)
            octaveR = 1; //R1
        else if (lastOctaveR > 1) {
            if (rollerMode)
                octaveR = lastOctaveR; //if rollers are released and we are not coming down from roller 1, stay at the higher octave
            if (otfKey && !rSum && polySelect && (polySelect < PolySelect::ERotatorA) && rotatorOn && (mainState == NOTE_OFF))
                hmzKey = fingeredNote % 12;
            if (mainState == NOTE_OFF)
                currentRotation = 3; //rotator reset by releasing rollers
        }
        if ((3 == rollerMode) && R1 && !R5 && (6 == lastOctaveR))
            octaveR = 7; // Bonus octave on top
        // Roller modes
        ...
        if ((3 == rollerMode) && R6 && !R5 && (6 == lastOctaveR))
            octaveR = 7; // Bonus octave on top
                         // Roller modes
                         // 0: Highest touched roller, no release memory (legacy style), 1 in menu
                         // 1: Highest touched roller, release memory, 2 in menu
    ...
    if (0 == fingering) { //EWI standard fingering
        ...
        while (matched < 10 && combo < 16) {
            ...
            for (byte finger = 0; finger < 10; finger++) {
                if ((saxFinger[finger] == saxFingerMatch[combo - 1][finger]) || (saxFingerMatch[combo - 1][finger] == 2))
                    matched++;
            }
        }
        if (matched < 11 && combo == 17)
            fingeredNoteUntransposed = lastFingering;
        else
            fingeredNoteUntransposed = startNote + 1 + saxFingerResult[combo - 1] - LHp2 + RHs - (RHp2 && (1 == combo) && LHp2) + octaveR * 12;
...
// Read touch pads (MPR121), compare against threshold value
    ...
    K2 = (touchValueRH[K2Pin] < ctouchThrVal);
    K3 = (touchValueRH[K3Pin] < ctouchThrVal);
    K5 = (touchValueRH[K5Pin] < ctouchThrVal);
    K6 = (touchValueRH[K6Pin] < ctouchThrVal);
    K7 = (touchValueRH[K7Pin] < ctouchThrVal);
    ...
    R1 = (touchValueRollers[R1Pin] < ctouchThrVal);
    R2 = (touchValueRollers[R2Pin] < ctouchThrVal);
    R3 = (touchValueRollers[R3Pin] < ctouchThrVal);
    R4 = (touchValueRollers[R4Pin] < ctouchThrVal);
    R5 = (touchValueRollers[R5Pin] < ctouchThrVal);
    rSum = R1 + R2 + R3 + R4 + R5;
    K4 = (touchValueRollers[K4Pin] < ctouchThrVal);
#else
    bool touchKeys[12];
    for (byte i = 0; i < 12; i++) {
    ...
    if (R5 && R3)
        ...
        octaveR = 4; //R4
    else if (R3 && (R2 || (oneroll && lastOctaveR)))
        octaveR = 3; //R3
    else if (R2 && (R1 || oneroll))
        octaveR = 2; //R2
    else if (R1)
        ...
        if (rollerMode)
            octaveR = lastOctaveR; //if rollers are released and we are not coming down from roller 1, stay at the higher octave (CV filter leak prevention when putting NuEVI aside)
        if (otfKey && !rSum && polySelect && (polySelect < PolySelect::ERotatorA) && rotatorOn && (mainState == NOTE_OFF))
            hmzKey = fingeredNote % 12;
        if (mainState == NOTE_OFF)
            currentRotation = 3; //rotator reset by releasing rollers
    }
    if ((3 == rollerMode) && R3 && !R5 && (6 == lastOctaveR))
        octaveR = 7; // Bonus octave on top
    // Roller modes
    // 0: Highest touched roller, no release memory (legacy style), 1 in menu
    // 1: Highest touched roller, release memory, 2 in menu
    ...
    if (0 == fingering) {                                               //EVI fingering
        fingeredNoteUntransposed = startNote - 2 * K1 - K2 - 3 * K3     //"Trumpet valves"
                                   - 5 * K4                             //Fifth key
