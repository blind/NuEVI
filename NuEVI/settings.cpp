...
bool receiveSysexSettings(const uint8_t *data, const uint16_t length) {
    ...
    for (uint16_t i = 0; i < payload_size / 2; i++) {
        ...
        if (addr == BREATH_THR_ADDR || addr == BREATH_MAX_ADDR) {
            if (val < breathLoLimit || val > breathHiLimit)
                continue;
        }

        if (addr == PORTAM_THR_ADDR || addr == PORTAM_MAX_ADDR) {
            if (val < portamLoLimit || val > portamHiLimit)
                continue;
        }

        if (addr == PITCHB_THR_ADDR || addr == PITCHB_MAX_ADDR) {
            if (val < pitchbLoLimit || val > pitchbHiLimit)
                continue;
        }

        if (addr == EXTRAC_THR_ADDR || addr == EXTRAC_MAX_ADDR) {
            if (val < extracLoLimit || val > extracHiLimit)
                continue;
        }

        if (addr == CTOUCH_THR_ADDR) {
            if (val < ctouchLoLimit || val > ctouchHiLimit)
                continue;
        }

        if (addr == LEVER_THR_ADDR || addr == LEVER_MAX_ADDR) {
            if (val < leverLoLimit || val > leverHiLimit)
                continue;
        }
...
void handleSysex(uint8_t *data, unsigned int length) {
    ...
    //Verify vendor
    if (strncmp((char *)(data + 1), sysex_id, 3))
        return; //Silently ignore different vendor id
    ...
    if (!strncmp(messageCode, "c00", 3)) { //Config dump request
        ...
        if (receiveSysexSettings(data + 1, length - 2))
            configShowMessage("New config saved.");
    } else {
...
bool resetSure(bool factoryReset) {
    ...
    int loopExit = 2;
    while (2 == loopExit) {
        if (digitalRead(ePin) && digitalRead(mPin) && digitalRead(dPin) && !digitalRead(uPin))
            loopExit = 1; //UP pressed
        if (digitalRead(ePin) && digitalRead(mPin) && !digitalRead(dPin) && digitalRead(uPin))
            loopExit = 0; //DOWN pressed
    }
    if (loopExit)
        display.println("RESETTING");
    else
        display.println("NO RESET - EXIT");
    display.display();
    delay(1000);
    logoDisplay();
