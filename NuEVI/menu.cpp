...
static void drawTrills() {
    if (LH1)
        display.fillRect(0, 0, 5, 5, WHITE);
    else
        display.drawRect(0, 0, 5, 5, WHITE);
    if (LH2)
        display.fillRect(10, 0, 5, 5, WHITE);
    else
        display.drawRect(10, 0, 5, 5, WHITE);
    if (LH3)
        display.fillRect(20, 0, 5, 5, WHITE);
    else
        display.drawRect(20, 0, 5, 5, WHITE);
}
#else
static void drawTrills() {
    if (K5)
...
const MenuEntrySub octaveMenu = {
    ...
    [](SubMenuRef __unused) { writeSetting(OCTAVE_ADDR, octave); }, nullptr};
...
const MenuEntrySub trill3Menu = {
    ...
    [](SubMenuRef __unused) { writeSetting(TRILL3_INTERVAL_ADDR, trill3_interval); }, nullptr};
...
const MenuEntrySub dacModeMenu = {
    ...
    [](SubMenuRef __unused) { writeSetting(DAC_MODE_ADDR, dacMode); }, nullptr};
...
const MenuEntrySub cvTuneMenu = {
    ...
    [](SubMenuRef __unused) { writeSetting(CVTUNE_ADDR, cvTune); }, nullptr};
...
const MenuEntrySub wlPowerMenu = {
    ...
    [](SubMenuRef __unused) { sendWLPower(wireless_power); }, nullptr};
...
const MenuEntrySub wlChannelMenu = {
    ...
    [](SubMenuRef __unused) { sendWLChannel(wireless_channel); }, nullptr};
...
const MenuEntrySub breathCC2RiseMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(BREATH_CC2_RISE_ADDR, breathCC2Rise); }, nullptr};
...
const MenuEntrySub velocityMenu = {
    ...
    [](SubMenuRef __unused, char *out, const char **__unused unit) {
        if (velocity)
            numToString(velocity, out);
        else
            strncpy(out, "DYN", 4);
    },
    [](const MenuEntrySub &__unused sub) { writeSetting(VELOCITY_ADDR, velocity); }, nullptr};
...
const MenuEntrySub velSmpDlMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(VEL_SMP_DL_ADDR, velSmpDl); }, nullptr};

const MenuEntrySub velBiasMenu = {
    MenuType::ESub, "VEL BOOST", "VEL BOOST", &velBias, 0, 9, MenuEntryFlags::EMenuEntryWrap,
    [](SubMenuRef __unused, char *out, const char **__unused unit) {
        if (velBias)
            numToString(velBias, out);
        else
            strncpy(out, "OFF", 4);
    },
    [](SubMenuRef __unused) { writeSetting(VEL_BIAS_ADDR, velBias); }, nullptr};
...
const MenuEntrySub breathIntervalMenu = {
    ...
    [](SubMenuRef __unused) { writeSetting(BRINTERV_ADDR, breathInterval); }, nullptr};
...
const MenuEntrySub biteCtlMenu = {
    ...
    [](SubMenuRef __unused, char *out, const char **__unused unit) {
        const char *labs[] = {"OFF", "VIB", "GLD", "CC", "VBCC", "GLCC", "VBGL", "ALL"};
        strncpy(out, labs[biteControl], 5);
    },
    [](SubMenuRef __unused sub) { writeSetting(BITECTL_ADDR, biteControl); }, nullptr};
...
const MenuEntrySub biteCCMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(BITECC_ADDR, biteCC); }, nullptr};
...
const MenuEntrySub leverCtlMenu = {
    ...
    [](SubMenuRef __unused sub) { writeSetting(LEVERCTL_ADDR, leverControl); }, nullptr};
...
const MenuEntrySub leverCCMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(LEVERCC_ADDR, leverCC); }, nullptr};
...
const MenuEntrySub stripCtlMenu = {
    ...
    [](SubMenuRef __unused sub) { writeSetting(STRIPCTL_ADDR, stripControl); }, nullptr};
...
const MenuEntrySub stripCCMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(STRIPCC_ADDR, stripCC); }, nullptr};
#endif
...
const MenuEntrySub portMenu = {
    ...
    [](SubMenuRef __unused sub) { writeSetting(PORTAM_ADDR, portamento); }, nullptr};
...
const MenuEntrySub portLimitMenu = {
    ...
    [](SubMenuRef __unused sub) { writeSetting(PORTLIMIT_ADDR, portLimit); }, nullptr};
...
const MenuEntrySub portLoLimitMenu = {
    ...
    [](SubMenuRef __unused sub) { writeSetting(PORT_LO_LIM_ADDR, portLoLimit); }, nullptr};
...
const MenuEntrySub pitchBendMenu = {
    ...
    [](SubMenuRef __unused) { writeSetting(PB_ADDR, PBdepth); }, nullptr};
...
const MenuEntrySub extraMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(EXTRA_ADDR, extraCT); }, nullptr};

const MenuEntrySub extraCC2Menu = {
    MenuType::ESub, "EXCT CC B", "EXCT CC B", &extraCT2, 0, 127, MenuEntryFlags::EMenuEntryWrap,
    [](SubMenuRef __unused, char *out, const char **__unused unit) {
        if (extraCT2)
            numToString(extraCT2, out);
        else
            strncpy(out, "OFF", 4);
    },
    [](const MenuEntrySub &__unused sub) { writeSetting(EXTRA2_ADDR, extraCT2); }, nullptr};

const MenuEntrySub harmonicsMenu = {
    MenuType::ESub, "EXCT HARM", "HARM RANGE", &harmSetting, 0, 6, MenuEntryFlags::EMenuEntryWrap,
    [](SubMenuRef __unused, char *out, const char **__unused unit) {
        if (harmSetting)
            numToString(harmSetting, out);
        else
            strncpy(out, "OFF", 4);
    },
    [](const MenuEntrySub &__unused sub) { writeSetting(HARMSET_ADDR, harmSetting); }, nullptr};
...
const MenuEntrySub deglitchMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(DEGLITCH_ADDR, deglitch); }, nullptr};
...
const MenuEntrySub pinkyMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(PINKY_KEY_ADDR, pinkySetting); }, nullptr};

const MenuEntrySub lvlCtrlCCMenu = {
    MenuType::ESub, "LEVEL CC", "LEVEL CC", &levelCC, 0, 127, MenuEntryFlags::EMenuEntryWrap,
    [](SubMenuRef __unused, char *out, const char **__unused unit) {
        if (levelCC)
            numToString(levelCC, out);
        else
            strncpy(out, "AT", 4);
    },
    [](const MenuEntrySub &__unused sub) { writeSetting(LEVEL_CC_ADDR, levelCC); }, nullptr};
...
const MenuEntrySub fingeringMenu = {
    ...
    [](SubMenuRef __unused sub) { writeSetting(FINGER_ADDR, fingering); }, nullptr};
#else
const MenuEntrySub fingeringMenu = {
...
const MenuEntrySub rollerMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(ROLLER_ADDR, rollerMode); }, nullptr};
...
const MenuEntrySub vibRetnMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(VIB_RETN_ADDR, vibRetn); }, nullptr};
...
const MenuEntrySub vibSenseMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(VIB_SENS_ADDR, vibSens); }, nullptr};
...
const MenuEntrySub vibSquelchMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(VIB_SQUELCH_ADDR, vibSquelch); }, nullptr};
...
const MenuEntrySub vibSenseBiteMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(VIB_SENS_BITE_ADDR, vibSensBite); }, nullptr};
...
const MenuEntrySub vibSquelchBiteMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(VIB_SQUELCH_BITE_ADDR, vibSquelchBite); }, nullptr};
...
const MenuEntrySub vibDirMenu = {
    ...
    [](const MenuEntrySub &__unused sub) { writeSetting(VIB_DIRECTION_ADDR, vibDirection); }, nullptr};
